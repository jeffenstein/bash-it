...
for __n in git hg svn
do
    if [[ -n "$(command which "$__n" 2>/dev/null)" ]];
    then
        eval "__${__n}=yes"
    else
        eval "__${__n}=no"
...
function __c {
    ...
    local __l="⁅"
    local __r="⁆"
    if [[ -n "$code" && -n "$string" ]]; then
        echo "\[${code}\]${__l}${string}${__r}\[${term_normal}\]"
    fi
}

function __my_hostname_prompt {
    if [[ -z "${HOST:-}" ]]; then
        HOST="$(uname -n | cut -d. -f1)"
    fi
    __c "${fg_green}" "$HOST${HOST_FRIENDLY:+(${HOST_FRIENDLY})}"
}

function __my_clock_prompt {
    __c "${fg_yellow}" "$(date '+%H:%M:%S')"
}

function __my_username_prompt {
    if [[ -z "${USER:-}" ]];then
...
function __my_last_status_prompt {
    local txt="rval:$1"
    if [[ "$1" -ne 0 ]]; then
        __c "${fg_red}" "$txt"
    # else
